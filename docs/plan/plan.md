...
- [ ] 右パネル（エディタ右側に常駐・開閉可能）。タブで機能を切替:
  ...
  - チャット
- [ ] チャット機能（lm-chat の ChatView が参考。2026-07-08 具体化）— 左=本文を見ながら右ペインで会話:
  - **7c-1 チャット MVP**: 現在のドキュメント（+ 選択範囲）を文脈として Gemma 4 とマルチターン対話。「この段落レビューして」「もっと簡潔な言い回しは？」等。ストリーミング表示。提案テキストの「カーソル位置に挿入」「選択範囲を置換」ボタン。backend に `/chat`（messages 配列 + document_md + selection を受けるマルチターン版。既存の client streaming を流用）を新設。
  - **7c-2 RAG トグル**: メッセージ送信ごとに RAG を使うか選べるトグル（既定 OFF。AssistPanel の「RAG」チェックボックスと同じ hybrid search を再利用）。goals.md の「明示制御」に従い、自動発火ではなくユーザーのトグルで制御する。
  - **7c-3 Web 検索トグル**: 会話中にネット検索して情報を取りに行くトグル（既定 OFF。フェーズ 4 のクエリ分解→取得→要約パイプラインを再利用。取得したソースはそのまま RAG 二層保存に載せ、以後の文脈に使えるようにする）。7c-1 より重いので後回し。
