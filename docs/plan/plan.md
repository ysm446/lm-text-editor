...
- [~] チャット機能（lm-chat の ChatView が参考。2026-07-08 具体化 → 7c-1 / 7c-2 実装済み）— 左=本文を見ながら右ペインで会話:
  - [x] **7c-1 チャット MVP**: 現在のドキュメント（+ 選択範囲）を文脈として Gemma 4 とマルチターン対話。ストリーミング表示。提案テキストの「挿入（カーソル位置）」「置換（選択範囲）」ボタン。backend `/chat`（messages 配列 + document_md + selection、既存 client streaming を流用）。右ペインは執筆支援/チャットのタブ切替（`ChatPanel.tsx`）。チャット履歴はエディタインスタンス内（ドキュメント単位・セッション限り。永続化は未実装）。
