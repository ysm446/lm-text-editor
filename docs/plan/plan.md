...
- [ ] 保存ボタン + Ctrl+S。未保存変更のインジケータ（タイトル横に ● など）
- [ ] `document_revision` テーブル（document_id, content_json, content_md, title, created_at）。保存時に document 更新 + リビジョン追加
- [ ] 履歴 UI: リビジョン一覧 → 現在との diff プレビュー（DiffText 流用）→ 復元
- [ ] 既存の 800ms デバウンス自動保存は廃止（フェーズ 1 実装分を置き換え）

実装時に決めること:
- クラッシュ・閉じ忘れ対策。明示保存のみだと編集内容が失われるため、別枠のドラフト自動保存（`draft_json` に退避し、次回オープン時に「未保存の変更を復元しますか？」）を入れるかどうか。
- リビジョンの保持方針（全部残す / 上限件数で間引き）。
- ドキュメント切替・アプリ終了時に未保存変更がある場合の確認ダイアログ。
