...
- 2026-08-11 アプリアイコンを作成し、Electron ウィンドウ／Windows タスクバーと favicon に適用。生成元は `assets/app-icon.png`（1024×1024 PNG）に集約した。

- 2026-07-30 プロジェクト全体のコードレビューと不具合修正（詳細は changelog 2026-07-30 19:42）:
  - フロント: IME 変換確定 Enter の誤発火（全入力欄）/ チャットのクリア×ストリーミング競合（世代管理）/ インライン校正の採用位置ずれガード / 保存とドラフト退避の競合（タイマークリアを await 前へ）/ 履歴読み込みの未保存確認 / 画像削除キャンセルでライトボックスが閉じる / streamText の TextDecoder 最終フラッシュ。
  - バックエンド: `/shutdown` が外部起動 llama-server を道連れにする問題（`manager.stop(takeover=False)` を新設。UI からの明示アンロードは従来どおり引き取り停止）/ `max_tokens_for()`・コンテキスト使用率の分母が設定値と稼働値を混同する問題（`llama_runtime.json` に起動時の thinking / context_length を記録して優先参照）/ 埋め込み未インストール時に hybrid_search が 500 になる問題（`embed_query` を try 内へ移し FTS のみで劣化継続）/ FTS の RRF 順位が登録順だった問題（`ORDER BY rank` 追加）/ `/web/ingest` の埋め込みエラーが「取得に失敗」と誤報告される問題（RuntimeError → 503）。
  - 検証: `npm run build` 通過 / 一時 machine root + 一時ライブラリのインプロセステスト（FTS フォールバック・takeover フラグ・稼働値優先とフォールバック・/web/ingest の 503）全通過。実 LLM・実アプリでの目視は未実施。
  - レビューで見つかったが今回未修正（要検討）: ノート更新の非原子性（本文更新後の埋め込み失敗でチャンクが空になり 500。保存自体は成功している）/ `llama_runtime.json` の read-modify-write 競合（ポーリングと起動の並走で稀に PID 追跡が外れる）/ ライブラリ切替中の並走リクエストの整合 / SourceViewer で資料切替時に未保存編集が黙って消える / ストリーミングの中断手段がない（AbortController 未導入。誤送信した長い生成を止められない）/ チャット応答中に上へスクロールできない（毎トークン最下部へ強制）/ ModelBar がロード中の backend クラッシュで「読み込み中…」のまま固まる / 「全体を校正」がリスト・引用・表の中の段落を黙ってスキップする（トップレベルのみ対象という既知の制限だが UI に表示がない）。

- 2026-07-30 チャットの質問候補チップ（ユーザー要望。story-graph の `ChatDrawer.tsx` / `docs/design/chat.md` §5・5.1 を参考）:
  - 置き方は story-graph 準拠: **会話の末尾（入力欄の直上）に右寄せで縦積み**し、会話と一緒にスクロールする（固定枠にすると会話エリアが常に狭くなる）。クリックで即送信。同時 5 件（`TEMPLATE_WINDOW`）+「⟳ ほかの候補」で決まった順に巡回（ランダムにしない）。応答中は無効化。
  - 定型質問（`ChatPanel.TEMPLATES` 12 種）は校正・改善寄りと構成・企画寄りを混ぜた。
  - 内容ベースの候補: `POST /chat/suggest_questions`（`document_md` の末尾 3000 字 + 直近 2 往復 → 最大 3 件）。**設定 OFF / LLM 未起動 / 生成失敗はすべて空配列**で、UI は定型のみになる。`_require_llm` は使わず `is_alive` を見るだけ（候補のためにモデルを起動しない）。生成は llama.cpp の `response_format`（JSON スキーマ）優先で、拒否されたら素のテキストで 1 度だけ再試行 → 行パース（前置き行を落とすため疑問文・依頼文の語尾だけ通す）。
  - 取得タイミング: チャットを開いたとき / 回答直後（会話を踏まえたフォローアップに差し替え）/ クリアしたとき。設定は `chat_dynamic_suggestions`（既定 ON、設定 > チャット）。
  - 検証: `npm run build` 通過 / `py_compile` OK / 一時 machine root + フェイク LLM のインプロセステスト（パーサの JSON・行出力・前置き除去・上限・思考タグ、設定 OFF で LLM を呼ばない、未起動で生成しない、材料なしで呼ばない、スキーマ拒否時の 1 回再試行、完全失敗でも空、プロンプトの末尾優先と履歴混入）通過。**実 LLM での候補の質は未確認**（:8080 が停止中だったため）。
...
- 2026-07-12 チャットの Web 検索トグル + チャット→資料（RAG）保存 + M ノート世代履歴（ユーザー要望。plan.md 7c-3〜7c-5）:
  - backend: `/chat` に `use_web` 追加。直近ユーザー発話を `websearch.search()`（クエリ分解 → ddgs/Tavily）にかけ、**スニペットのみ**を文脈注入（ページ取得・要約・RAG 自動保存はしない = テンポと明示制御優先。7c-3 の元案から設計変更・ユーザー合意）。NDJSON の先頭に `{"sources": [...], "queries": [...]}` を 1 行流す。検索失敗はログして検索なしで続行。
  - backend: `POST /rag/note/{id}/merge`（既存ノート + 新情報を LLM で統合した Markdown を返す。**保存はしない** — プレビュー確認後にフロントが PUT）。統合プロンプト（`NOTE_MERGE_SYSTEM`）に「既存記述を削らない・『## 参考』の URL/日付を蓄積する」を焼き込み。`<think>` は strip_think で除去。
  - backend: `manual_note_revision` テーブル追加（update_note 時に旧本文を退避。内容変更時のみ・ノートごと 20 世代・ノート/ワークスペース削除でカスケード）。`GET /rag/note/{id}/revisions`（一覧）/ `GET /rag/note/{id}/revisions/{rev_id}`（本文）。
  - frontend: チャット入力行に「Web」トグルボタン（RAG と同スタイル・既定 OFF）。返答下に出典 URL リンク表示。各返答のメタ行に「資料へ保存」→ モーダル（`SaveChatNoteModal.tsx`）で①新規 M ノート（タイトル = 直前の質問から prefill、出典 + 取得日を「## 参考」に自動整形）②既存 M ノートへ追記（統合案生成 → 編集可能プレビュー → 上書き保存）。追記先は M ノート限定（原文系資料に LLM 文章を混ぜない）。
  - frontend: SourceViewer（M ノート編集）ヘッダに「履歴」→ 世代一覧 → クリックで編集エリアへ読み込み（未保存変更ありは confirm）→「保存」で復元確定。
  - 検証: `npm run build` 通過 / `py_compile` OK。一時ライブラリのインプロセステストで ①世代履歴（作成→更新→退避、タイトルのみ変更では増えない、20 世代キャップ、note_id 不一致ガード、削除カスケード）②/chat の NDJSON プロトコル（TestClient + フェイク LLM/検索: sources 行が先頭、web_context がプロンプトに入る、use_web=false は従来互換、検索失敗でも 200 継続）③merge（統合案が返る・元ノート未変更・think 除去）を全て確認。実 LLM・実ネットでの目視は未実施（`LM_KEEP_BACKEND=1 npm run dev` で予定）。
...
- フェーズ 7 残り: Web 検索クエリ分解の文脈対応（7c-6・掘り下げ質問で無関係な検索をしてしまう）、トピックノートの出典肥大化対策（7c-7・URL デデュープ / 出典の取捨選択 / 参考セクションのチャンク除外）、履歴の永続化、アドバイス/ソースのタブ、分割ビュー校正の右パネル寄せ、ノート側からの「再検索して更新」入口。チャット MVP + RAG トグルは 2026-07-08、Web 検索トグル + 資料保存 + 世代履歴は 2026-07-12 実装済み。plan.md フェーズ 7 参照。
- 表・チャット（Web トグル・資料保存・履歴含む）の実アプリ目視確認（`LM_KEEP_BACKEND=1 npm run dev` で backend を巻き込まずに）。
