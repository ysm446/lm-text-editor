...
**フェーズ 2（LLM 接続と校正）進行中**。インライン校正と執筆支援（続き生成・セクション生成）が API レベルで動作（Gemma 4 実測済み）。残りは分割ビュー校正としきい値設定。
...
- UI の手動確認: 執筆・保存・画像ペースト・インライン校正・執筆支援の一連操作。
- フェーズ 2 の残り: 分割ビュー校正（左右分割、段落対応付け）、しきい値設定（2 段落以上で分割ビュー）。
