...
- LLM は 1 つに一本化（:8080）。以前は検索用に ornith 9B を :8081 に分けていたが、検索・要約も文章用モデルに統合した。
- モデル呼び出しはタスクルータ（`backend/router.py`）が仲介する疎結合。
...
- **データはライブラリ単位**: DB（文書・RAG）と画像は「ライブラリ」フォルダに保存（左サイドバー下部のライブラリメニューで新規作成・切替。Obsidian の Vault 相当）。既定は `data/`（gitignore 済み）。**設定・レジストリ・llama-server の PID 記録も `data/`**（配布前のため。`machine_root()` = `data/`。旧 `~/.lm-text-editor/` の settings/libraries は初回に自動移行）。パスは必ず `backend/paths.py` の関数経由で解決する（import 時に固定すると切り替えが効かない）。
- `start.bat` — backend（別ウィンドウ）+ Vite + Electron の一括起動。ASCII のみで書く（cmd はシステムコードページで .bat を解釈するため日本語コメント禁止）。
- `start-llm.bat` — llama-server で Gemma 4 を 127.0.0.1:8080 に起動（ロードに 1〜2 分）。**通常は UI 上部のモデルピルから起動/停止する**（`backend/llm/manager.py` が subprocess 管理）。bat 等で外部起動した / 追跡が切れて居残ったサーバも、モデルピル → モーダルの「アンロード」やモデル切替でアプリから停止・引き取りできる（`_stop_on_port`: netstat で自ポートの PID を特定し、プロセス名が llama-server の場合のみ kill）。
- **注意**: Gemma 4 は reasoning モデル。思考の有無は**設定（⚙️ → LLM →「思考モード」）**で切り替える（既定 OFF）。`backend/llm/manager.py` が起動引数を組み立て、OFF は `--reasoning-budget 0`（テンプレートの `enable_thinking=false` になる）、ON は `--reasoning on --reasoning-format deepseek`（思考は content ではなく `reasoning_content` に分離）。**リクエストの `chat_template_kwargs.enable_thinking` では上書きできない**（実測。起動引数が優先）ため、変更はモデル再起動が必要。思考 ON のときは `manager.max_tokens_for()` が猶予（`THINK_HEADROOM`）を足す — これを忘れると思考が max_tokens を食い潰し、HTTP 200 なのに content が空になる。
- **LLM のモデル割り当ては設定（⚙️ → LLM）**: models/ から 1 つ選ぶ（`writing_model_path`）。執筆・校正・画像・Web 検索のクエリ分解・要約すべてにこのモデルを使う（単一 LLM に一本化。以前の検索用 ornith スロットと `search_model_path` は廃止）。
- **Web 検索**: ddgs / DuckDuckGo（キー不要）が既定。`TAVILY_API_KEY`（環境変数または `data/settings.json` の `tavily_api_key`）があれば Tavily を優先。クエリ分解・要約は文章用 LLM（:8080）が起動していれば使い、いなければ元クエリのまま検索・要約なしで続行する。
...
- **書き込みを伴う検証は、ユーザーの実データに対して絶対に行わない。** 一時ディレクトリに `paths.set_library_root()` したインプロセステストで行う。アクティブライブラリの DB・レジストリ（`data/libraries.json`）に書き込みテストをしない。
