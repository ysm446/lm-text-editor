...
UI に関わる作業のときは、加えて `docs/design/style-guide.md`（UI スタイルガイド）を読む。

## UI 変更時のルール

**UI に関わる変更は必ず `docs/design/style-guide.md` に従う。** アートスタイルの一貫性を保つため、例外なく適用する。

- 対象は、見た目・レイアウト・配色・文字・余白・アニメーションに影響するすべての変更。新規コンポーネントに限らず、既存画面の調整や一箇所だけの修正も含む。
- 色・文字サイズ・角丸・余白・影・アニメーション時間は、スタイルガイドで定義済みの値（`src/styles.css` のトークン / 既定の段階）から選ぶ。新しい値や新種の部品を思いつきで追加しない。
- ガイドから外れる必要がある場合は、**実装より先に `docs/design/style-guide.md` を更新して規則として明文化してから**実装する。
- 作業を終える前に、スタイルガイド末尾のチェックリストを通す。
